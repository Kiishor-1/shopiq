export const footerData = [
  {
    title: "SHOP BY CATEGORY",
    links: [
      "Electronics",
      "Furniture",
      "Fashion",
      "Groceries",
      "Health & Wellness",
      "Books & Stationery",
      "Toys & Games",
    ],
  },
  {
    title: "CUSTOMER SUPPORT",
    links: [
      "Help Center",
      "Returns & Refunds",
      "Shipping Information",
      "Order Tracking",
      "Terms of Service",
      "Privacy Policy",
      "Feedback & Suggestions",
    ],
  },
  {
    title: "DOWNLOAD OUR APP",
    links: [],
    images: [
      { src: "/images/stores.svg", alt: "Google Play & Play Store" },
    ],
    social: [
      { src: "/facebook-random.svg", alt: "Facebook" },
      { src: "/twitter-random.svg", alt: "Twitter" },
      { src: "/youtube-random.svg", alt: "YouTube" },
      { src: "/instagram-random.svg", alt: "Instagram" },
    ],
  },
  {
    title: "INFORMATION",
    links: [
      "About Us",
      "Investor Relations",
      "Affiliate Program",
      "Gift Cards",
      "Corporate Sales",
      "Partnerships",
      "Sustainability",
    ],
  },
  {
    title: "TRENDING SEARCHES",
    links: [],
    text: [
      "Smartphones",
      "Laptops",
      "Bluetooth Speakers",
      "Watches",
      "Sunglasses",
      "Running Shoes",
      "Cookware Sets",
      "Bed Linens",
      "Graphic T-Shirts",
      "Backpacks",
      "Yoga Mats",
      "Air Fryers",
      "Winter Jackets",
      "Travel Accessories",
      "Ethnic Wear",
      "Wireless Earbuds",
      "Leather Wallets",
      "Skincare Products",
      "Home Decor",
    ],
  },
];


export const slidesData = [
  { id: 1, title: "#Fashionera - Deal of the Year Edition", bg: "bg-yellow-400", image: "https://res.cloudinary.com/dkvrdo3z1/image/upload/v1734544127/img1_kwitcj.png" },
  { id: 2, title: "Save up to 35% on perfumes & frangrances", bg: "bg-blue-600", image: "https://res.cloudinary.com/dkvrdo3z1/image/upload/v1734544127/img2_i2cj7d.png" },
  { id: 3, title: "Loot on Beauty Products", bg: "bg-red-500", image: "https://res.cloudinary.com/dkvrdo3z1/image/upload/v1734544130/img3_yqeixt.png" },
  { id: 4, title: "Mega Discounts on Handbags & Heels", bg: "bg-green-500", image: "https://res.cloudinary.com/dkvrdo3z1/image/upload/v1734544130/img4_vrcwhw.png" },
  { id: 5, title: "Mega Sale on Fashionable Jeans", bg: "bg-purple-500", image: "https://res.cloudinary.com/dkvrdo3z1/image/upload/v1734544129/img5_reviwa.png" },
];